...
def calculate_changed_coverage(change: GitChange, coverage: CoverageData) -> CoverageData:
    if coverage.error or not coverage.executed_lines:
        return CoverageData(coverage.global_percent, coverage.files, error=coverage.error or "dados de linhas ausentes", executed_lines=coverage.executed_lines or {})
    changed = []
    covered = []
    ...
    return CoverageData(coverage.global_percent, coverage.files, changed_percent=percent, error=coverage.error, executed_lines=coverage.executed_lines)
